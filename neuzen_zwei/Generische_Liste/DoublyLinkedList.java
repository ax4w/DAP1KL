package Klausur.neuzen_zwei.Generische_Liste;
...
public class DoublyLinkedList<T>
{
    ...
    //-------------------------------------------------------
    // Klausur 2019_2
    public boolean containsAll(Iterable<T> data) {
        Element f = first;
        while (f != null) {
            Klausur.Iterator<T> di = data.iterator();
            boolean found = false;
            while (di.hasNext()) if(di.next().equals(f.getContent())) found = true;
    ...
    public void insert(DoublyLinkedList<T> list, T cont) {
        ...
        while (f != null && !f.getContent().equals(cont)) f = f.getSucc();
        while (dl.getSucc() != null) dl = dl.getSucc();
        if(f == null) return;
        if(f == first) {
            first.connectAsPred(dl);
